<script>
    ...
    const { name, description, latitude, longitude, isPrivate } = newLandmark;
    ...
    const newLandmarkData = { name, description, latitude, longitude, imageUrls, isPrivate };

    // Attempt to create the landmark in Firebase
    if (await createLandmark(newLandmarkData, categoryId)) {
        newLandmark = { name: '', description: '', latitude: null, longitude: null, isPrivate: false };
    ...
    <div class="box">
                    ...
                    <div class="content">
                        ...
                        {#if landmark.imageUrls}
                            {#each landmark.imageUrls as url}
                                <img src={url} alt={`Image of ${landmark.name}`} style="width: 100px; height: auto;">
                            {/each}
                        {/if}
                        <div class="buttons">
        ...
        <form on:submit|preventDefault={addNewLandmark}>
            ...
            <label class="switch">
                ...
                <span class="slider round"></span> Is Private
            </label>
